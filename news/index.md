...
- Removed paths related to pipeline caches. We no longer cache outputs
  from pipelines, because we never used them.
- Deleted some tests.
...
- Added path the database schema setup file, by default called
  `SchemaAndSimpleTables.xlsx`.
- One new test.
...
- [`pin_versions()`](https://energyeconomydecoupling.github.io/PFUSetup/reference/pin_versions.md)
  now moved to package PFUPipelineTools.
...
- Added new functions
  [`pin_versions()`](https://energyeconomydecoupling.github.io/PFUSetup/reference/pin_versions.md)
  and `read_pin_version()` to assist development of computational
  pipelines.
- New tests for new functions.
...
- Relative paths if `home_path` and `cloud_storage_path` are both set to
  an empty string (`""`).
...
- Now defaulting to 2022 release of IEA World Extended Energy Balance
  (WEEB) data.
- No new tests.
...
- Eliminated references to drake cache path. We no longer use the
  `drake` package.
- Added constants required by the `MWTools` package for Muscle Work
  analysis.
- No new tests.
...
- New release, same code and functionality as 0.1.9, whose only purpose
  is to fix DOI problems. Year on the license has been fixed, too.
...
- Added `phi_constants` table to output of
  [`get_abs_paths()`](https://energyeconomydecoupling.github.io/PFUSetup/reference/get_abs_paths.md).
- Exemplar table now sits in Mapping folder. Changed
  [`get_abs_paths()`](https://energyeconomydecoupling.github.io/PFUSetup/reference/get_abs_paths.md)
...
- Added `drake_cache_path` to the list of strings returned from
  [`get_abs_paths()`](https://energyeconomydecoupling.github.io/PFUSetup/reference/get_abs_paths.md).
- No new tests.
  - Now at 22 tests, all passing.
  - Test coverage remains at 100 %.
...
- Now skips file existence tests if running on continuous integration
  systems.
...
- Added `reports_source_folder` and `reports_dest_folder` items in the
  output of
  [`get_abs_paths()`](https://energyeconomydecoupling.github.io/PFUSetup/reference/get_abs_paths.md).
